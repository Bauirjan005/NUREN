  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 min-h-screen overflow-x-hidden">
      ...
      <div className="bg-white border-b-2 border-slate-200 p-4 sticky top-0 z-50 flex items-center space-x-4 shadow-sm">
        ...
        <div>
          <h1 className="text-2xl font-black text-green-500 uppercase tracking-tight">Reading Path</h1>
      ...
      <div className="flex-1 py-12 flex flex-col items-center relative overflow-hidden">
        ...
        {lessons.map((lesson, index) => {
          const unlocked = isLessonUnlocked('reading', lesson.id);
          const completed = completedLessons.includes(`reading-${lesson.id}`);
          ...
          return (
            <div 
              key={lesson.id} 
              onClick={() => unlocked ? navigate(`/lesson/reading/${lesson.id}`) : null}
              className="relative z-10 flex flex-col items-center mb-8 group"
              style={{ marginLeft: margin }}
            >
              {/* Floating Title (Only for Unlocked/Active node) */}
              {!completed && unlocked && (
                <div className="absolute -top-12 bg-white px-4 py-2 rounded-2xl border-2 border-slate-200 shadow-sm animate-bounce z-20 whitespace-nowrap">
                  <span className="font-black text-green-500 uppercase tracking-wider text-sm">Start</span>
                  {/* Tooltip triangle */}
              ...
              {/* The Node Button */}
              <div 
                className={`w-24 h-24 rounded-full flex items-center justify-center border-b-8 shadow-lg transition-all ${bgColor} ${borderColor} ${textColor} ${unlocked ? 'cursor-pointer hover:scale-105 active:border-b-0 active:translate-y-2' : 'cursor-not-allowed opacity-80'}`}
              >
                {icon}
              </div>
            </div>
